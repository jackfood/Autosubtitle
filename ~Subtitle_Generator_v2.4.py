                    ...
                    if is_single_chunk_mode is None:
                        if (match := re.search(r'VAD found (\d+) speech segments', final_part)):
                            ...
                            if total_chunks > 1:
                                ...
                                self.master.after(0, lambda tc=total_chunks: [self.file_progress.config(maximum=tc), self.overall_progress_label.config(text=f"Overall Progress: {i+1} / {num_files}")])
                            else:
